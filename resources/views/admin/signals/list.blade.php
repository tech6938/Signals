...
<div class="content-wrapper">
  <section class="content-header">
    ...
    <div class="header-title">
      <h1>{{ __('messages.signals') }}</h1>
      <small>{{ __('messages.signals_table') }}</small>
      <ol class="breadcrumb hidden-xs">
        <li><a href="{{ route('dashboard') }}"><i class="pe-7s-home"></i> {{ __('messages.home') }}</a></li>
        <li class="active">{{ __('messages.signals') }}</li>
      </ol>
    </div>
  </section>
          ...
          <div class="panel-heading">
            <div class="btn-group">
              <a class="btn btn-primary" href="{{ route('signals.create') }}">
                <i class="fa fa-plus"></i> {{ __('messages.add_signal') }}
              </a>
            </div>
          </div>
              ...
              <table class="table table-bordered table-hover">
                <thead>
                  <tr>
                    ...
                    <th>{{ __('messages.b_price') }}</th>
                    <th>{{ __('messages.tp1') }}</th>
                    <th>{{ __('messages.tp2') }}</th>
                ...
                <tbody id="tableBody">
                  ...
                  <tr>
                    ...
                    <td>{{ rtrim(rtrim(number_format($signal->b_price, 4, '.', ''), '0'), '.') }}</td>
                    <td>{{ rtrim(rtrim(number_format($signal->tp1, 4, '.', ''), '0'), '.') }}</td>
                    <td>{{ rtrim(rtrim(number_format($signal->tp2, 4, '.', ''), '0'), '.') }}</td>
                    <td>
                      @if($signal->icon1)
                    ...
                    <td>{{ rtrim(rtrim(number_format($signal->last_price, 4, '.', ''), '0'), '.') }}</td>
