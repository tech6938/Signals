        ...
        <td>
            <div class="btn-group">
                <button type="button" class="btn btn-info btn-sm" data-toggle="modal"
                    data-target="#viewModal{{ $notification->id }}">
                    <i class="fa fa-eye"></i>
                </button>
                <!-- @if (!$notification->sent)
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            @if ($notification->send_to === 'individual' && $notification->target_users)
                                @php
                                    $targetUsersArray = is_string($notification->target_users)
                                        ? json_decode($notification->target_users, true)
                                        : $notification->target_users;
                                    $users = \App\Models\User::whereIn('id', $targetUsersArray)->get();
                                @endphp
                                @if ($users->count() > 0)
                                    <div class="form-group">
                                        ...
                                        <p>
                                            @foreach ($users as $user)
                                                <span class="label label-default">{{ $user->f_name }}
                                                    {{ $user->last_name }}</span>
                                            @endforeach
                                        </p>
                                    </div>
