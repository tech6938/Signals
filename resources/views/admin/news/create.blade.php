...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <div class="header-title">
      ...
      <small>{{ __('messages.add_new_news') }}</small>
      <ol class="breadcrumb hidden-xs">
        <li><a href="{{ route('dashboard') }}"><i class="pe-7s-home"></i> {{ __('messages.home') }}</a></li>
        <li class="active">{{ __('messages.create_news') }}</li>
      </ol>
    </div>
  </section>
          ...
          <div class="panel-heading">
            <div class="btn-group">
              <a class="btn btn-success" href="{{ route('news.index') }}">
                <i class="fa fa-table"></i> {{ __('messages.news_table') }}
              </a>
            </div>
          </div>

          <div class="panel-body">
            <form class="col-sm-12" action="{{ route('news.store') }}" method="POST" enctype="multipart/form-data">
              @csrf
...
<div class="form-group col-md-6 mb-3">
  ...
  <select class="form-control" name="audience_type" id="audience_type" onchange="toggleAudienceField()">
    <option value="all" {{ old('audience_type') == 'all' ? 'selected' : '' }}>{{ __('messages.all_users') }}</option>
    <option value="subscribers" {{ old('audience_type') == 'subscribers' ? 'selected' : '' }}>{{ __('messages.subscribers_only') }}</option>
    <option value="package" {{ old('audience_type') == 'package' ? 'selected' : '' }}>{{ __('messages.subscribers_of_package') }}</option>
  </select>
</div>
  ...
  <select class="form-control" name="package_id" id="package_id">
    ...
    @endforeach
              ...
              <div class="col-sm-12 reset-button">
                ...
                <button type="submit" class="btn btn-success">{{ __('messages.add_news') }}</button>
              </div>
...
// Auto run when the page loads (in case old input is set)
